import React from 'react';
import Image from 'next/image';
const GhostwritingBanner = (props) => {
 


  return (
    <section className='w-full bg-white text-[#1d1d1f] py-4 font-secondary'>
     <div className='relative '>
      <div
            className="absolute -z-0 md:hidden sm:hidden xs:hidden top-0 right-0 w-[1000px] h-[750px] bg-no-repeat bg-right md:bg-center"
            style={{ backgroundImage: "url('/assets/images/newimg/genre-banner-new.png') " }}
          ></div>
      </div>
       
        <div className='max-w-7xl container mx-auto text-left justify-center items-center py-36 md:py-16 sm:py-10 xs:py-10 ' >
          <div className='flex md:grid xs:grid xs:grid-cols-1 md:mx-auto md:w-[90%] md:text-center' >
            <div className='basis  sm:w-[100%] xs:w-[100%] xs:py-0 xs:px-6 md:text-center xs:text-center'>
              <h1 className='w-[90%] md:w-[100%] font-bold leading-[60px] text-[52px] xs:text-3xl sm:w-[100%] xs:w-[100%]'>
                {props.title}
              </h1>
              <p className='w-[75%] mt-3  md:w-[100%] sm:w-[100%] xs:w-[100%]'>
                {props.dec}
              </p>
              <div className='w-[50%] grid md:mx-auto grid-cols-2 grid-rows-1 gap-x-5 py-5 sm:w-[100%] xs:w-[100%]'>
              <button className='bg-[#00C0E4] py-1 text-white hover:bg-transparent hover:border-solid hover:border-[1px] hover:border-[#1d1d1f] hover:text-[#1d1d1f] hover:transition-all hover:transform-gpu'>Get Started</button>
                  <button className='bg-[#1d1d1f] py-1 text-white border-solid border-[#1d1d1f] border-[1px] hover:bg-[#00C0E4] hover:text-[#fff] hover:border-[1px] hover:border-[#00C0E4]'>Live Chat</button>
              </div>
            </div>

            <div className='xl:hidden 2xl:hidden lg:hidden mx-auto justify-center items-center text-center md:pb-0 xs:w-[90%] xs:pb-0 md:mt-12 xs:mt-12 ' >
              <Image src={'/assets/images/newimg/genre-banner-new.png'} width={800} height={600} alt='medium-magazine-writing' />
            </div>
            <div className='mx-auto w-[100%] md:hidden sm:hidden xs:hidden justify-center items-center text-center pb-20 xs:w-[90%] xs:pb-10 ' >

            </div>
          </div>



        </div>
      

     

      </section>
  );
};

export default GhostwritingBanner;
